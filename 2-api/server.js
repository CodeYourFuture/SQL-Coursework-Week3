...
app.get('/customers/:customerId', (req, res) => {
  ...
  if (!isValid) {
    ...
    .then((result) => {
      ...
      if (!doesExist) {
        res.status(404).json({ 'Server message': `A customer by the id ${id} does not exist!` });
      } else {
        pool
          .query(query)
...
app.post('/customers', async (req, res) => {
  ...
  const customersInsertQuery = 'INSERT INTO customers (name, address, city, country) VALUES ($1, $2, $3, $4)';
  try {
    await pool.query(customersInsertQuery, [name, address, city, country]);
    res.json({ Success: `Customer by the name ${name} is successfully registered!` });
  } catch (error) {
    res.status(500).send(error);
  }
});
...
app.post('/products', async (req, res) => {
  ...
  try {
    ...
    res.json({ Success: `Product by the name ${productName} is successfully inserted!` });
  } catch (error) {
    res.status(500).send(error);
  }
});
...
app.post('/availability', async (req, res) => {
  ...
  const availabilityInsertQuery = 'INSERT INTO product_availability (prod_id, supp_id, unit_price) VALUES ($1, $2, $3);';
  const existsQuery = 'select exists(select 1 from product_availability where supp_id=$1 AND prod_id=$2);';
  ...
  if (!isValid(supplierId) || !isValid(productId) || !isValid(price)) {
    ...
    try {
      ...
      const suppliersResult = await pool.query(supplierExistsQuery, [supplierId]);
      ...
      if (doesExist) {
        ...
        pool.query(availabilityInsertQuery, [productId, supplierId, price])
...
app.post('/customers/:customerId/orders', async (req, res) => {
  ...
  const customerOrderQuery = 'INSERT INTO orders (order_date, order_reference, customer_id) VALUES ($1, $2, $3)';
  ...
  if (!isValid(customerId)) {
    ...
    try {
      ...
      if (!doesExist) {
        ...
        pool.query(customerOrderQuery, [orderDate, orderReference, customerId])
          .then(result => res.json({ Success: `Order is successfully inserted!` }))
          .catch(error => console.error(error))
      }
    } catch (error) {
      res.status(500).send(error);
  ...
  if (!isValid(id)) {
    ...
    pool.query(checkIfExists)
      .then((result) => {
        ...
        let doesExist = exists.pop();
        if (!doesExist) {
          res.status(404).json({ 'Server message': `An order by the id ${id} does not exist!` });
        } else {
          pool.query(deleteItemsQuery)
    ...
    pool.query(checkIfExists)
      .then((result) => {
        ...
        if (!doesExist) {
          ...
          pool.query(checkIfOrdered)
            .then((result) => {
              if (result.rows.length > 0) {
                res.json({ 'Server message': `A customer by the id ${id} has orders and can not be deleted!` });
              } else {
...
app.get('/customers/:customerId/orders', async (req, res) => {
  ...
  const checkIfExistsQuery = 'SELECT EXISTS(SELECT 1 FROM customers WHERE id=$1)';
  ...
  if (!isValid(id)) {
    ...
    try {
      ...
      if (!doesExist) {
        ...
        pool.query(customerOrdersQuery)
          .then(result => res.json(result.rows))
          .catch(error => console.error(error))
      }
    } catch (error) {
